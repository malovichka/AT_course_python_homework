...
def _match_print(collection: list[str], root: str, name: str | None = None):
...
def find(folder, name=None, show_dirs=True, show_files=True):
    ...
    for root, dirs, files in os.walk(folder):
        if show_dirs:
            _match_print(dirs, root, name)
        if show_files:
            _match_print(files, root, name)
...
def parse_cmd_args():
    ...
    parser.add_argument(
        "-type", nargs="?", default=None, choices=["f", "d"], help=type_help
    )
